def extract_extended_end_utr(bed_list_raw, chrom_sizes_dict, tmp_dir):
    
    utr_size = 10100
    output_write = open(tmp_dir+'extended_end_utr.bed', 'w')
    ...
    for fields in bed_list_raw:
        if '_' not in fields[0]:
            ...
            if strand == '+':
                tx_start = str(int(tx_end) - int(exon_sizes.strip(',').split(',')[-1]))
