...
temp_dir = '/data/home/luskry/polyadenylation_ML_algorithm/algorithm_code/machine_learning/ml/beijing/'
gold_standard_rna_seq_sorted_bam_file = temp_dir+'sorted_beijing_uhr_ensembl_v91_index_default_hisat2.bam'
...
gold_standard_gtf_file_from_rna_seq_data = temp_dir+'beijing_uhr_ensembl_v91_genome_index_default_GRCh38.v91.txt'
    ...
    try:
        ...
        for curr_3UTR_id in UTR_events_dict:
            ...
            for i in range(len(curr_3UTR_all_samples_bp_coverage)):
                ...
                cur_depth = All_Samples_Total_depth[i]
                idxs_sums.append(get_polya_seq_peaks_and_sums(probe_region, thres = cur_depth/1E6))
                probe_region = np.zeros(len(probe_region))
                idxs = [[t[0] for t in l] for l in idxs_sums][i]
                sums = [[t[1] for t in l] for l in idxs_sums][i]
                if idxs != []:
                    for x, y in zip(idxs, sums):
                        probe_region[x] = y
                    comb_probes.append(probe_region)
            if comb_probes != []:
                indicator_list = convert_polya_seq_to_indicator(sum(comb_probes))
                indicator_list_fin = (' '.join([str(int(item)) for item in indicator_list]))
              
            output_write.writelines('\t'.join([str(curr_3UTR_id), ' '.join(indicator_list_fin)])+'\n')
